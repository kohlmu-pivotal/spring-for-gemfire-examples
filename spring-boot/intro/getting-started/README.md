...
## Description
In this example we will show you how to get started using GemFire with Spring Boot.
...
2. Start GemFire server-side processes
    1. Open new Terminal
    2. `cd` into the `intro/getting-started` project directory.
    3. Start `gfsh` located in the `{VMware_GemFire_installation_location}/bin` directory
    4. Launch the Locator and Server by running `start.gfsh` file located in the `getting-started` project directory in `gfsh`. <br> e.g `run --file={pathToStartGfshFile}/start.gfsh`
    5. Confirm that the locator and server are running by running `list members` in `gfsh`, which should contain two entries.
3. Start Client
    1. Open a new Terminal
    2. `cd` into the `getting-started` project directory
...
```
Saving Customer [Customer[id=1, name=JonDoe]]...
Querying for Customer [SELECT * FROM /Customers WHERE name LIKE '%Doe']...
Customer was [Customer[id=1, name=JonDoe]]
```

You can then hit `http://localhost:8080/customers` and see JonDoe as the only entry.
